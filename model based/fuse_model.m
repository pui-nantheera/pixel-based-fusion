function FUSEDCOEF = fuse_model(ORIGIMGS, LEVELS, HIGH, LOW, BIORT, QSHIFT, GAIN, FINETUNE)
%   fuse_model - Image fusion with Dual-tree complex wavelet transfrom
%   and Cauchy probability
%
%   FUSEDCOEF = fuse_model(ORIGIMGS, LEVELS, HIGH, LOW, BIORT, QSHIFT, GAIN, FINETUNE)
%   ORIGIMGS is a 3-D matrix: input images
%   LEVELS is maximum decomposition level   (default = 4)
%   HIGH is coefficient selection highpass according to defined model (see selcModel.m)
%           HIGH == 1: Weigted average scheme
%           HIGH == 2: FLOM algorithm
%           HIGH == 3: FLOM-Cauchy algorithm 
%           HIGH == 4: Meridian distribution
%           HIGH == 5: Cauchy probability
...
%               high = 3;   % use FLOM algorithm
%               % Using default biort and qshift, 
%               F = fuse_model(I, levels, high, low);
%               figure; imshow(F)
%               % Add sharpening
%               gain = [2 1.4 1.1 1]; 
%               F = fuse_model(I, levels, high, low, [], [], gain);
%               figure; imshow(F)
...
for scale = 1:LEVELS
    for band = 1:6
        ...
        Outh{scale}(:,:,band) = selcModel(COEFS, HIGH);
