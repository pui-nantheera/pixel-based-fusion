function [SNR_DWT,c] = DWTSNR(I0,I1,J,wavf,recon,c)
...
rc= size(I0); rc = rc(1:2);
k = 1:rc(1); % spatial indices 
l = 1:rc(2);

if nargin < 2 | size(I0,3) == 2, 
  I1 = I0(:,:,2); I0 = I0(:,:,1);   
end
...
if nargin < 6, 
  KK = 1 : 2; % coefficients has not been supplied
elseif isempty(c), KK = 1 : 2;
else KK = find(isnan(c(1,1,:))); KK = KK(:)';
end
...
for K = KK % orignal first, then distorted
