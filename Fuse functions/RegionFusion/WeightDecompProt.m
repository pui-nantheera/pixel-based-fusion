function Y = WeightDecompProt(WeightingMask, LabSeg, Levels, ImSize)
...
%Levels = 4;
%LabSeg = LoadIm('h:\images\2d\mmsegfus\ir+viz\1809labi.tif');
%imview(LabSeg,[]);
%ImSize = size(LabSeg);
%pwr2 = mod(ImSize(1), 2^Levels);
%if (pwr2 ~= 0)
%    LabSeg(ImSize(1):ImSize(1)+2^Levels-pwr2, :) = 0;
%end;
%pwr2 = mod(ImSize(2), 2^Levels);
%if (pwr2 ~= 0)
%    LabSeg(:,ImSize(2):ImSize(2)+2^Levels-pwr2) = 0;
%end;
%figure, imshow(LabSeg,[]);

Y = cell(Levels, 1);
levsize = ceil(ImSize./2);
for a = 1:Levels
    Y(a, :) = { zeros(levsize(1), levsize(2), 6) };    
...
%figure, imshow(Y{1}, []);
for a = 2:Levels
    clear teven; clear dif; clear mask;
